---
layout: default
title: GIS Portfolio
show_profile: false
---

# GIS Portfolio

<div class="portfolio-wrapper">

  <a href="/projects/wui-mapping" class="portfolio-card"
     style="background-image: url('/img/GIS_background.png');">
    <span>A Tool for Mapping Wildland Urban Interface</span>
  </a>

  <a href="/projects/salv-army" class="portfolio-card"
     style="background-image: url('/img/salv-army.png');">
    <span>GIS Projects for the Salvation Army</span>
  </a>

  <a href="https://sites.google.com/colorado.edu/geog4093finalproject/home?authuser=1" class="portfolio-card"
      class="portfolio-card title-bottom"
      style="background-image: url('/img/glacier_change.gif');">
    <span>Remotely Sensing Glacial Retreat and Snowpack Levels in Saas Fee, Switzerland</span>
  </a>

  <a href="/projects/teton-glaciers" class="portfolio-card"
     style="background-image: url('/img/teton-glaciers.png');">
    <span>Spatial Analysis of the Glaciers in Grant Teton National Park</span>
  </a>

  <a href="/projects/cartography" class="portfolio-card"
     style="background-image: url('/img/cartography.png');">
    <span>Cartography Examples</span>
  </a>

  <a href="/projects/Group-Project.html" class="portfolio-card"
     style="background-image: url('/img/statistics.png');">
    <span>Statistics with Geographic Data (R){#statistics}</span>
  </a>

  
