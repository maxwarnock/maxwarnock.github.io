# Cartography Examples {#cartography}

<img src="/maps/Map_Layout_lab09.jpg" alt="portland1" width="1650">
Population density maps of the Portland Tri-Metro area shown with different choropleth classification methods. Certain methods are useful for different data distributions. In this case, the quantile method shows the most difference between zones. 
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/lab08_map.jpg" alt="portland2" width="1650">
Map of the Portland Tri-Metro area showing the airport, parks, summits, water bodies, general topography, and the boundary of the Tri-Metro area. 
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/Lab07_map.jpg" alt="portland3" width="1650">
Map of the Portland Tri-Metro area showing roads.
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/Warnock_lab4_map.png" alt="gulfport1" width="1650">
Map of Gulfport, MS, showing storm surge areas based on elevation as well as emergency resources. 
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/Warnock_lab3_map.png" alt="denver1" width="1650">
In this figure, the top two maps compare the locations of high income households (>200k), and low income households (<10k) in Denver, CO. The bottom left map shows locations of affordable housing, and the bottom right shows a zoomed in snapshot of education levels within central Denver. Data for these maps was taken from the U.S. Census and the Denver Regional Council of Governments (DRCOG). 
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/Warnock.png" alt="tillamook1" width="1275">
Map showing locations of school-aged children in Tillamook County, OR. This map could be used to help determine locations for new schools. 
<hr style="height: 2px; background-color: grey; border: none; margin: 20px 0;">

<img src="/maps/Warnock_lab1_map.png" alt="tillamook2" width="1275">
Map showing land ownership, railroads, cities, and flood zones in Tillamook County, OR. This map could be used for general planning purposes. 


<hr style="height: 4px; background-color: black; border: none; margin: 20px 0;">
